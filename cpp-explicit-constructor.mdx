...
The `explicit` specifier prevents the compiler from treating a constructor as a converting constructor. To illustrate what a converting constructor is and how it is used, consider the following class definition:
...
In a specific renderer, the alpha channel is unsupported and will be ignored in its `fill_something` method. To prevent confusion for callers accidentally specifying an alpha value or interpreting its omission as a bug, we introduce a new class `RGB` without an alpha channel. This is defined as follows:
