...
Thus a caller of the `fill_something` is supposed to have a clear view that the alpha channel will be ignored by writing:
...
However, suprisingly, following invocation does **NOT** lead to compiler errors; instead the `fill_something` is 