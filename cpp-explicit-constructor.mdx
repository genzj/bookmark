...
It invalidates our design scope that a caller should be aware of alpha dropping when writing the code. The reason that above invocation works is because the _implicit conversion_: the compiler finds the `RGB(const RGBA& rgba)` takes one parameter of `RGBA&` and
