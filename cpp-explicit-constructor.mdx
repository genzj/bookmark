...
As a general practice, do not define implicit conversions by using the `explicit` keyword for conversion operators and single-argument constructors, unless for rare exceptiona types that are designed to be interchangeable (ref: [Implicit Conversions, Google C++ Style Guide](https://google.github.io/styleguide/cppguide.html#Implicit_Conversions)).